...
vars.con <- c("lo_immigrant_dup","stdlagconabst_pol2_dup","terror_nr",
...
freq_table(wno_data, admin42)

#Models 1 and 2 in Table 5 of the Appendix
  #Model 1
model.1 <- plm(lo_immigrant_dup ~ terror_nr,
               data = wno_data,
               index = c("org2","year"),
               model = "within")
...
mat.1 <- perm_table(data = wno_data, model = model.1,
  ...
  #Model 2
model.2 <- plm(lo_immigrant_dup ~ stdlagconabst_pol2_dup*terror_nr,
               data = wno_data,
               index = c("org2","year"),
               model = "within")
...
lapply(fit, function(x) sd(wno_data$lo_immigrant_dup - x$fitted.values)) #rmse
